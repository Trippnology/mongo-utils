...
// ponytail: bare db names ("somedb") aren't valid URLs; anchor them on localhost
parseURL = function (connectionString) {
	try {
		return new URL(connectionString);
	} catch (_error) {
		return new URL('mongodb://localhost/' + connectionString);
	}
};
...
utils.parseConnectionString = function (connectionString) {
	...
	if (parsedURL.username) {
		info.username = parsedURL.username;
		info.password = parsedURL.password;
	}
	return info;
};
